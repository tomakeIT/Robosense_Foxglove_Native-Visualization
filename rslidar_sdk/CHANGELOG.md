...
## v1.5.18-foxglove.1 2026-03-16

### Added

- Native C++ Foxglove WebSocket publishing path in `rslidar_sdk_node`.
- Cross-platform Foxglove SDK selection for macOS and Linux (`x86_64` / `aarch64`).
- Runtime tuning knobs for low-latency and low-loss modes:
  - split frame controls: `split_frame_mode`, `split_angle`, `num_blks_split`
  - queue policy: `low_latency_drop_old_frames`, `max_pending_frames`
  - memory preallocation: `prealloc_points`, `prealloc_cloud_msgs`, `prealloc_points_per_msg`

### Changed

- Removed Python UDP bridge dependency from default runtime path.
- Updated startup script to launch C++ node directly.
- Updated config and docs for Foxglove-native architecture.
